// -------------Starting Wallet Management Engine-------------------
...
// -------------Authentication Wallet Management Engine----------------
function tryLogin() {
...
auth.onAuthStateChanged(user => {
    if (user) {
        ...
        document.getElementById('dashboard').style.none = 'none';
    }
});

function adminLogout() { auth.signOut(); }

// --------------Dashboard Wallet Management Engine-----------------
...
// ---------Investigation Logic Wallet Management Engine------------
...
// -------------Powerful Copy Wallet Management Engine-------------------
function copyAddr(text) {
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(text).then(() => {
            smatPopup("Address Copied Successfully!", "SUCCESS");
...
function fallbackCopy(text) {
    ...
    try {
        ...
        smatPopup("Failed to copy. Please copy manually.", "FAILED");
    }
    document.body.removeChild(textArea);
}

// -------------Action For Wallet Management Engine-------------------
async function approve(uid, tid) {
    const h = document.getElementById('h_'+tid).value;
    if(!h || h.length < 5) return smatPopup("Valid TrxHash is required!", "FAILED");
    await db.ref(`users/${uid}/history/${tid}`).update({ status:'success', hash:h });
    smatPopup("Transaction Approved!", "SUCCESS");
}
...
async function reject(uid, tid, coin, amt) {
    if(confirm("Are you sure you want to Reject and Refund?")) {
        try {
            ...
            if(coinLower === 'usdt') {
           // ------KeyBalance Wallet Management Engine----------- 
            }
            ...
            smatPopup("Transaction Rejected & Funds Refunded", "SUCCESS");
        } catch (e) {
            console.error("Reject Error: ", e);
            smatPopup("Error occurred while refunding.", "FAILED");
        }
    }
}

// ----------Tost Notification Wallet Management Engine--------------
function smatPopup(message, title = "SUCCESS") {
    let wrapper = document.querySelector('.smat-toast-wrapper');
    if (!wrapper) {
        wrapper = document.createElement('div');
        wrapper.className = 'smat-toast-wrapper';
        document.body.appendChild(wrapper);
    }

    const toast = document.createElement('div');
    toast.className = 'smat-toast';
    
    const icon = title === "SUCCESS" ? "fa-circle-check" : "fa-triangle-exclamation";

    toast.innerHTML = `
        <i class="fa-solid ${icon}"></i>
        <div class="smat-toast-content">
            <span class="smat-toast-title">${title}</span>
            <span class="smat-toast-msg">${message}</span>
        </div>
    `;
...
// ---------------Ending Wallet Management Engine--------------------